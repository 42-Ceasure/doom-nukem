...
int			parse_line(t_map *m)
{
	...
	if (m->section_number == 1)
	{
		if (ft_strcmp(tmp[0], "\tdots") == 0)
		{
			...
			while (tmp2[i] != NULL)
			{
				m->dot[m->i].x = ft_atof(tmp2[i]);		// coder ft_ft_atof
				m->dot[m->i].y = ft_atof(tmp[1]);		// coder ft_ft_atof
				m->i++;
				i++;
			}
			free(tmp2);
	...
	if (m->section_number == 2)
	{
		if (ft_strcmp(tmp[0], "\tplayer_location") == 0)
		{
			...
			m->player.coor.x = ft_atof(tmp1[0]);		// coder ft_ft_atof
			m->player.coor.y = ft_atof(tmp1[1]);		// coder ft_ft_atof
		}
