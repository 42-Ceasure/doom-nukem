...
static void DrawScreen(void)
{
    ...
    *head = (struct item) { player.sector, 0, W-1 };
    ...
    while(head != tail)
    {
        ...
        SDL_UnlockSurface(surface);
        SaveFrame2(surface); SDL_UpdateWindowSurface(window);
        SDL_LockSurface(surface);*/
    ...
    SaveFrame2();
    //static unsigned skip=0;
    //if(/*++skip >= 10 && */!map) { skip=0; SDL_UpdateWindowSurface(window); }
}
...
int main(int argc, char** argv)
{
    ...
    sdlWindow = SDL_CreateWindow("SDL2 Example", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, W2, H, 0);
...
#endif
        static unsigned skip=0;
        if(++skip >= 1) { skip=0; SDL_UpdateWindowSurface(sdlWindow); }
