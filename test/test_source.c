...
static SDL_Window* surface = NULL;
...
//    surface = SDL_SetVideoMode(W2, H, 32, 0);
    surface = SDL_CreateWindowAndRenderer(W2, H, SDL_WINDOW_RESIZABLE, &surface, &renderer);
...
#endif
        static unsigned skip=0;
        if(++skip >= 1) { skip=0; SDL_RenderPresent(surface); }
